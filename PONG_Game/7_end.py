#  end part at teh 10 or we can take from the user
#  also the returning with function done
import sys
...
def new():
    #  for the constant screen after winning check each and every time before update
    test = True
    ...
    while run:
        ...
        #  checkng the end point
        if paddle_1.points == 10:
            pong.speed = 0
            font = pygame.font.SysFont('Comic Sans MS', 21)
            text = font.render('GAME OVER!   Player 1 Win', False, deep)
            textRect = text.get_rect()
            textRect.center = (750//2,370//2)
            win.blit(text, textRect)

            #  drawing the box
            box = pygame.draw.rect(win, col, (300,3*500//7,150,500//7))


            #  over the box
            font = pygame.font.SysFont('Comic Sans MS', 21)
            text = font.render('Menu', False, deep)
            textRect = text.get_rect()
            textRect.center = (750//2,500//2)
            win.blit(text, textRect)

            if test == True:
                pygame.display.update()
            
            #  not require to update
            test= False
        ...
        if paddle_2.points == 10:
            pong.speed = 0
            font = pygame.font.SysFont('Comic Sans MS', 21)
            text = font.render('GAME OVER!   Player 2 Win', False, deep)
            textRect = text.get_rect()
            textRect.center = (750//2,370//2)
            win.blit(text, textRect)
